...
public class PickUpCounter extends HudElement implements PlayerObserver {
	...
	public void onPickUpCountChanged (Player player, Class<? extends PickUp> pickUpClass, int value)
	{
		if(value > 0)
		{
			int count = player.getPickUpCount(pickUpCls) + player.getPickUpCount(pickUpCls2) + player.getPickUpCount(pickUpCls3)-1; //-1 because the counter has already one PickUpRocket at the beginning of the Game
